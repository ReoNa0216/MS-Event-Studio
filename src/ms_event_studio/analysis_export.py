...
def export_analysis(project, parent, *, binding, result_id, include_pending):
    ...
    name = f'analysis-{uuid.uuid4().hex[:12]}.zip'
    ...
    with tempfile.TemporaryDirectory(prefix='.ms-analysis-', dir=parent) as temp:
        ...
        with zipfile.ZipFile(archive_path, 'w', compression=zipfile.ZIP_DEFLATED) as archive:
            ...
            archive.writestr('analysis_record.json', json.dumps(metadata, ensure_ascii=False, indent=2))
            if record:
    ...
    return dict(kind='review_results', display_name=name, row_count=csv.row_count, sha256=digest,
                message='已导出事件表及 Feature 矩阵。' if record else '已导出事件表；本次不含 Feature 矩阵。')
