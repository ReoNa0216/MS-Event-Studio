...
当前公开测试版本为 `0.4.0`。桌面界面使用单一的 pywebview + HTML/CSS/SVG 渲染器，
...
Windows 主流程已经过用户人工验收。`0.4.0` 在此基础上新增跨平台导出修复、当前窗口安全
...
- 新建项目时设置主 marker m/z 和“相邻事件距离较近”的提示阈值；核心质量窗口固定为 ±12 ppm；
...
- 先预览影响，再安全应用分析范围变化；
- 一次保留当前窗口内除相邻风险外的未审阅事件，并作为一个操作撤销或重做；
